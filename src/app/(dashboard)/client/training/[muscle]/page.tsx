        ...
        <div className="lg:col-span-8 space-y-6 w-full min-w-0">
          ...
          <div className="bg-[#0A0A0A] rounded-3xl border border-white/5 overflow-hidden shadow-2xl relative">
            ...
            <div className="p-4 md:p-6 bg-gradient-to-t from-black via-black/80 to-transparent">
              <Badge className={cn(
                "px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest border-none mb-2",
                activeExercise.difficulty === 'beginner' ? "bg-green-500/20 text-green-400" :
                activeExercise.difficulty === 'intermediate' ? "bg-yellow-500/20 text-yellow-400" :
                "bg-red-500/20 text-red-400"
              )}>
                {getDifficultyLabel(activeExercise.difficulty)}
              </Badge>
              <h2 className="text-2xl md:text-3xl font-black text-white uppercase italic tracking-tight truncate">{activeExercise.name}</h2>
            </div>
          </div>

          {/* MOBILE ONLY: Quick Exercise Switcher */}
          <div className="lg:hidden w-full space-y-3">
            <div className="flex items-center justify-between px-1">
              <h3 className="text-[10px] font-black text-white uppercase tracking-widest">Explorar ejercicios</h3>
              <span className="text-[10px] text-zinc-500 font-bold bg-white/5 px-2 py-0.5 rounded-full">{muscleInfo.exercises.length}</span>
            </div>
          ...
          <Card className="bg-[#111111] border-white/5 rounded-3xl overflow-hidden shadow-2xl">
            ...
            <CardHeader className="pb-2">
              <CardTitle className="text-zinc-500 text-[10px] uppercase tracking-[0.3em] font-black">
        ...
        <div className="hidden lg:block lg:col-span-4 space-y-6 lg:sticky lg:top-8">
          <div className="space-y-4">
            <div className="flex items-center justify-between px-2">
              <h3 className="text-xs font-black text-white uppercase tracking-widest">Más ejercicios</h3>
              <Badge variant="outline" className="text-[10px] border-white/10 text-zinc-500">{muscleInfo.exercises.length}</Badge>
            </div>
              ...
              {muscleInfo.exercises.map((exercise, idx) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <span className="text-[9px] text-zinc-600 uppercase font-bold">{getDifficultyLabel(exercise.difficulty)}</span>
                  </div>
                </button>
              ))}
            </div>
          ...
          <Card className="bg-zinc-900/40 border-white/5 rounded-2xl">
            <CardContent className="p-4">
              <p className="text-[10px] uppercase tracking-widest text-zinc-600 font-bold mb-2 font-black">Sobre {muscleInfo.name}</p>
              <p className="text-[11px] text-zinc-500 leading-relaxed italic">{muscleInfo.description.substring(0, 100)}...</p>
            </CardContent>
          </Card>
        </div>
      </div>
